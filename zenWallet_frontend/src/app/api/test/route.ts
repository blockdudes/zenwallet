...
export const POST = async (request: any) => {
	try {
		...
		const erc20TransferSignature = 'Transfer(address,address,address,uint256)';
        ...
        webhookData.event.data.block.logs.forEach(async (log: any) => {
			if (log.topics.includes(erc20TransferSignatureHash)) {
				...
				const tokenAddress = abiCoder.decode(['address'], log.topics[3])[0];
				const amount = abiCoder.decode(['uint256'], log.data)[0];

                console.log(fromAddress, toAddress, tokenAddress, amount)
				...
				if (tokenAddress === ethers.ZeroAddress) { 
					...
					toChatId && await sendMessage(toChatId, `You have received ${amount.toString()} of token at address ${tokenAddress}.`);
				}
			}
		});
